...
public readonly struct TuiColor : IEquatable<TuiColor>
{
    ...
    // Optimization: Substituted string.Substring and string.Split with ReadOnlySpan<char> slicing and MemoryExtensions.Split.
    // Time Complexity: O(N) where N is the length of the string to parse.
    // Space Complexity: O(1) by utilizing stackalloc Range for the output splits.
    private static TuiColor ParseFunctional(string text)
    {
        ...
        int partCount = inside.Split(ranges, ',');

        if (hasAlpha && partCount != 4)
            throw new FormatException($"rgba() requires 4 components: '{text}'.");
        if (!hasAlpha && partCount != 3)
