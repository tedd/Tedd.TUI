...
public readonly struct TuiColor : IEquatable<TuiColor>
{
    ...
    /// <summary>
    /// Parses rgb() and rgba() functional color representations.
    /// Time Complexity: O(n) where n is the length of the color string (constant bounded).
    /// Space Complexity: O(1) - Uses stackalloc for span splitting, eliminating all heap allocations.
    /// </summary>
