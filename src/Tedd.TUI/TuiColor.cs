...
public readonly struct TuiColor : IEquatable<TuiColor>
{
    ...
    // Optimization: Parsing rgb() and rgba() without intermediate string allocations.
    // Time Complexity: O(N) where N is the length of the string component to be parsed.
    // Space Complexity: O(1) by using stackalloc Range for Split and ReadOnlySpan<char> slicing.
    private static TuiColor ParseFunctional(ReadOnlySpan<char> textSpan)
    {
        int open = textSpan.IndexOf('(');
        int close = textSpan.IndexOf(')');
        if (open < 0 || close < 0 || close <= open)
            throw new FormatException($"Malformed color '{textSpan.ToString()}'.");

        bool hasAlpha = textSpan.Slice(0, open).Trim().Equals("rgba", StringComparison.OrdinalIgnoreCase);
        var inside = textSpan.Slice(open + 1, close - open - 1);

        Span<Range> ranges = stackalloc Range[5]; // Up to 5 segments to detect errors
        int count = inside.Split(ranges, ',');

        if (hasAlpha && count != 4)
            throw new FormatException($"rgba() requires 4 components: '{textSpan.ToString()}'.");
        if (!hasAlpha && count != 3)
            throw new FormatException($"rgb() requires 3 components: '{textSpan.ToString()}'.");
