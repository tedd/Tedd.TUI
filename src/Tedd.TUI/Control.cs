...
public class Control : UIElement
{
    ...
    // Cached sets to track trigger evaluation
    ...
    protected override void OnPropertyChanged(DependencyProperty dp)
    {
        ...
        {
            // Clear any active trigger values from the old template before applying the new one
            if (_activeTriggers != null && _activeTriggers.Count > 0)
            {
                foreach (var triggerBase in _activeTriggers)
                {
    ...
    private void EvaluateTriggers()
    {
        ...
        // If there are no triggers to evaluate, skip entirely
        if (!hasTriggers && (_activeTriggers == null || _activeTriggers.Count == 0))
        {
            _activeTriggers?.Clear();
            return;
        }
