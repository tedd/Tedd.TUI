...
## 2026-03-13 - Thumb Primitive Integration
**Observation:** The TUI framework lacked the `Thumb` primitive control, forcing composite controls like `ScrollBar` and `Slider` to hardcode drag behaviors and rendering logic, which fundamentally violated WPF structural isomorphism and template modularity. The required drag-related routed events (`DragStarted`, `DragDelta`, `DragCompleted`) were also missing from the standard API surface.
**Strategic Action:** Developed the `Thumb` primitive control inheriting from `Control`. Integrated WPF-parity drag interaction mechanics utilizing standard `MouseEventArgs` translations to absolute offset changes, backed by standard bubbling routed events (`DragStartedEvent`, `DragDeltaEvent`, `DragCompletedEvent`) using proper `DragEventArgs`. Leveraging `TuiWindow.CaptureMouse`, this unblocks correct declarative `ControlTemplate` generation for `ScrollBar` and `Slider`.
