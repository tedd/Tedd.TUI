...
**Observation:** The TUI framework lacked a native `Thumb` primitive control, a fundamental building block in WPF for draggable elements (e.g., within `Slider` or `ScrollBar`). Its absence necessitated duplicated drag lifecycle management directly within consuming controls.
**Strategic Action:** Developed the `Thumb` primitive inheriting from `Control`. Registered WPF-equivalent bubbling routed events: `DragStartedEvent`, `DragDeltaEvent`, and `DragCompletedEvent`. Integrated custom `RoutedEventArgs` derived classes overriding `InvokeEventHandler` to map to explicit typed delegates, and managed state using `IsDraggingProperty` coupled with global mouse capture via `TuiWindow.CaptureMouse`.
