...
## 2026-03-09 - Thumb Component Parity Integration
**Observation:** The TUI framework lacked the fundamental `Thumb` primitive control, a core structural component in standard client frameworks (like WPF) used for composing draggable elements such as sliders, scrollbars, and resizers. The absence of this component created a gap in WPF parity, where standardized bubbling drag events are expected.
**Strategic Action:** Engineered the `Thumb` primitive control inheriting from `Control`. Implemented the state machine to track dragging status via mouse capture (`TuiWindow.CaptureMouse`), and registered the standard `RoutingStrategy.Bubble` events: `DragStartedEvent`, `DragDeltaEvent`, and `DragCompletedEvent`. This resolves the architectural parity gap by providing a foundational component that accurately tracks and propagates horizontal and vertical drag transformations.
