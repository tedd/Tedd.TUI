...
- Implemented the standard .NET event wrapper for `ValueChanged`.
- Updated the `Value` property setter to track the old value and raise the `ValueChangedEvent` when the clamped value explicitly changes, ensuring robust routed event infrastructure.
