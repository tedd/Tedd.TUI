...
## 2024-05-24 - Unbound Generic Type `nameof` Operators
**Observation:** Numerous properties registered using `DependencyProperty.Register` and `DependencyProperty.RegisterAttached` utilized hardcoded string literals for property names (e.g., `"Left"`, `"Width"`). This pattern from earlier .NET versions is fragile against refactoring and increases the likelihood of runtime errors due to string typos.
**Strategic Action:** Applied C# 14 unbound generic type `nameof` operator across the `Tedd.TUI` codebase to replace fragile string literals in `DependencyProperty.Register` and `DependencyProperty.RegisterAttached` calls. By utilizing `nameof(PropertyName)`, we ensure compiler validation of property names, reducing bugs during refactoring and completely eliminating string-based lookup vulnerabilities. This enforces the modernized C# 14 compile-time checking philosophy.
