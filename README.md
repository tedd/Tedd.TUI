...
- **Rich Control Suite:**
  ...
  - **Slider:** Supports horizontal and vertical orientations. Provides a bubbling `ValueChanged` routed event when the clamped value explicitly changes, ensuring WPF event routing parity.
  - **Thumb:** Inherits from `Control` and provides standard WPF-equivalent bubbling routed events for drag lifecycles: `DragStarted`, `DragDelta`, and `DragCompleted`.
