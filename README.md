...
- **Rich Control Suite:**
  ...
  - **Standard Controls:** `Button`, `TextBox`, `PasswordBox`, `TextEditor`, `CheckBox`, `RadioButton`, `ProgressBar`, `TabControl`, `ListBox`, `ComboBox`, `GroupBox`, `TreeView`, `TreeViewItem`, `HeaderedItemsControl`, `Expander`, `DialogBox`, `UniformGrid`, `ScrollViewer`, `ScrollBar`, `Thumb`, `Slider`, `MenuBar`, `MenuItem`.
  - **MenuBar:** Inherits from `StackPanel` and provides a horizontal topological root for `MenuItem` elements. It manages global visual state (e.g., background rendering) and acts as the anchor for drop-down hierarchical navigation.
  - **MenuItem:** Encapsulates hierarchical execution and navigation logic. It dynamically leverages `TuiWindow.PushOverlay` to render flyout submenus (`ListBox` hosted in `Border`), managing recursive input routing and focus traversal to achieve desktop-equivalent cascading interactions.
