...
- **Rich Control Suite:**
  ...
  - **Standard Controls:** `Button`, `TextBox`, `PasswordBox`, `TextEditor`, `CheckBox`, `RadioButton`, `ProgressBar`, `TabControl`, `ListBox`, `ComboBox`, `GroupBox`, `TreeView`, `TreeViewItem`, `HeaderedItemsControl`, `Expander`, `DialogBox`, `UniformGrid`, `ScrollViewer`, `ScrollBar`, `Thumb`, `Slider`, `MenuBar`, `MenuItem`, `GridSplitter`.
  - **MenuBar:** Inherits from `StackPanel` to provide a horizontal topological root for `MenuItem` elements, managing global visual state and acting as an anchor for drop-down hierarchical navigation.
  - **MenuItem:** Encapsulates hierarchical execution and navigation logic, dynamically leveraging `TuiWindow.PushOverlay` to render flyout submenus (`ListBox` hosted in `Border`), managing recursive input routing and focus traversal.
  - **GridSplitter:** Inherits from `Thumb` and uses the `DragDelta` routed event to dynamically adjust the `GridLength` of adjacent `RowDefinition` and `ColumnDefinition` elements.
