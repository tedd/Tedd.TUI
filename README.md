...
- **Rich Control Suite:**
  ...
  - **Standard Controls:** `Button`, `TextBox`, `PasswordBox`, `TextEditor`, `CheckBox`, `RadioButton`, `ProgressBar`, `TabControl`, `ListBox`, `ComboBox`, `GroupBox`, `TreeView`, `TreeViewItem`, `HeaderedItemsControl`, `Expander`, `DialogBox`, `UniformGrid`, `ScrollViewer`, `ScrollBar`, `Slider`.
  - **Slider:** Inherits from `UIElement` and implements a bubbling `ValueChanged` routed event (`ValueChangedEvent`) that is explicitly raised from the `Value` property setter only when the clamped value explicitly mutates, guaranteeing WPF event routing structural parity.
...
- **Execution Phases:** `UIElement` implements two-phase input event routing. The `UIElement.RaiseEvent` pipeline utilizes a zero-allocation algorithm leveraging `System.Buffers.ArrayPool<UIElement>` to construct a discrete routing table with $O(1)$ allocation overhead by walking `Parent` references. Tunneling events prefixed with 'Preview' (e.g., `PreviewKeyDownEvent`) are dispatched first, sequentially from root to leaf; controls can mark these as `Handled` and, by convention, choose not to raise the corresponding standard bubbling event for WPF-style interception semantics. Subsequently, `Bubble` events trace backwards from leaf to root. When `Handled = true` is set on the event args, `UIElement.RaiseEvent` still traverses the full bubble route, but normal instance handlers are suppressed unless they were registered with `handledEventsToo = true`, while class handlers (virtual `OnEvent` / `OnXxx` methods) continue to run, mirroring WPF's routed event behavior. Input event dispatching across the framework is strictly delineated for WPF parity: `TuiWindow` autonomously manages the two-phase dispatch of keyboard events, whereas `ConsoleInputManager` governs the two-phase dispatch of mouse events.
