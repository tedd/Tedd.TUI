...
- **Rich Control Suite:**
  ...
  - **ListBox:** Achieves WPF architectural isomorphism by utilizing a `ControlTemplate` wrapping an `ItemsPresenter` within a `ScrollViewer`, removing custom `MeasureOverride` and `Render` logic and deferring layout to standard XAML paradigms. The generated container, `ListBoxItem`, inherits from `ContentControl` and manages its visual state via the `IsSelected` dependency property and `Selected`/`Unselected` bubbling routed events. Additionally, `Selector.SelectionChanged` is implemented as a `RoutedEvent` rather than a standard CLR event, and `SelectionChangedEventArgs` inherits from `RoutedEventArgs`.
  ...
  - **ItemsControl:** Inherits from `Control` and utilizes an `ItemsPanel` property (`ItemsPanelTemplate`) alongside an `ItemsPresenter` to generate and populate its visual layout panel, aligning with WPF logical separation. It supports dynamic data mapping via `ItemTemplate` (`DataTemplate`), automatically propagating the ambient data object to a generated `ContentPresenter` during `PrepareContainerForItemOverride` (additionally, note that `FrameworkTemplate.LoadContent` accepts a `DependencyObject templatedParent`). `ContentControl` exposes a `HasContent` dependency property (and `HeaderedContentControl` and `HeaderedItemsControl` expose `HasHeader`) that automatically updates based on content presence, matching WPF parity and providing support for declarative template triggers.
...
- **Visual Tree:** A hierarchical structure of elements allowing for complex composition. Any `UIElement` can dynamically ascend the visual tree to resolve its root host via the `GetRoot()` API, enabling recursive topological queries. `UIElement.Visibility` uses the standard WPF three-state `Visibility` enum (`Visible`, `Hidden`, `Collapsed`). `Collapsed` bypasses layout allocation in `Measure` and `Arrange`, whereas `Hidden` allocates layout space but is skipped during `Render` and hit testing.
...
(`TuiXamlView`), Avalonia, WinUI and MAUI hosts. The `XamlLoader.ConvertValue` method supports parsing strings into `DateTime` and `TimeSpan` types, enabling declarative configurations for controls like `DatePicker` and `TimePicker`. See the [XAML guide](docs/xaml.md).
