...
- **DataContext Inheritance:** The `DataContext` property is an explicitly defined inherited dependency property. The `DependencyObject` base architecture deterministically resolves hierarchical values by recursively querying the `InheritanceParent` (which reliably maps to the logical `Parent` within the `UIElement` hierarchy). Consequently, assigning a `DataContext` instance at the visual root (e.g., `TuiWindow`) guarantees downward propagation of the data model throughout the entire visual tree. This propagation is autonomously enforced via `GetVisualChild` enumeration logic, which synchronously triggers dependency property invalidations on all child nodes, establishing absolute synchronization parity.
...
- **Execution Phases:** `UIElement` strictly enforces a two-phase input event routing architecture. The `UIElement.RaiseEvent` pipeline is fundamentally designed around a zero-allocation execution flow, leveraging `System.Buffers.ArrayPool<UIElement>` to dynamically compile a discrete routing table by ascending the visual tree via `Parent` references with $O(1)$ allocation overhead. During execution, tunneling events (prefixed with 'Preview', such as `PreviewKeyDownEvent`) are deterministically dispatched first, traversing sequentially from the root visual node down to the target leaf. If a listener marks a preview event as `Handled`, the framework immediately intercepts the flow and prevents the subsequent, paired non-preview bubbling event from initiating. Conversely, `Bubble` events trace the inverse path from the leaf origin backward to the root. If `Handled = true` is asserted during the bubbling phase, the traversal route strictly continues ascending to the root node; however, standard instance handlers that were not explicitly registered with the `handledEventsToo = true` parameter are systematically bypassed, while overridden class handlers and explicitly configured instance handlers will still execute, ensuring absolute predictability in event interception.
