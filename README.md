...
- **High Performance:** Designed with a "Zero-Allocation" rendering philosophy, utilizing `Span<char>`, `stackalloc`, and double-buffered `VirtualBuffer` diffing to minimize I/O and GC pressure. `TuiColor` string parsing logic (e.g., handling CSS-style `rgba()` and `rgb()`) is optimized using `ReadOnlySpan<char>` slicing and `MemoryExtensions.Split` with a `stackalloc Range` buffer to eliminate intermediate string allocations and GC overhead.
...
- **Visual Tree:** A hierarchical structure of elements allowing for complex composition. Any `UIElement` can dynamically ascend the visual tree to resolve its root host via the `GetRoot()` API, enabling recursive topological queries. Standard WPF visual tree traversal is supported via the `VisualTreeHelper` static class, which maps `GetChildrenCount`, `GetChild`, and `GetParent` to the underlying `UIElement` properties `VisualChildrenCount`, `GetVisualChild`, and `Parent`.
- **Visibility:** `UIElement.Visibility` uses the standard WPF three-state `Visibility` enum (`Visible`, `Hidden`, `Collapsed`). `Collapsed` bypasses layout allocation in `Measure`/`Arrange`, whereas `Hidden` allocates layout space but is skipped during `Render` and hit testing.
...
- **Color Parameters:** Component wrappers (such as `TuiButton` or `TuiLabel`) commonly expose color parameters as standard `ConsoleColor` values, which support implicit conversion to the core library's `TuiColor` struct for unified rendering.
...
Tedd.TUI supports defining UI in XAML. You can load UI elements dynamically from XML strings at runtime using `Tedd.TUI.XamlLoader.Load(xml, controller)`. This mechanism automatically resolves logical elements and dynamically binds named controls to the provided controller instance via reflection. The `XamlLoader`'s `ConvertValue` method supports parsing strings into `DateTime` and `TimeSpan` types, enabling declarative configurations for controls like `DatePicker` and `TimePicker`.
