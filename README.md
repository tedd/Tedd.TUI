...
### Layout Engine
The framework employs a robust, recursive two-pass layout system orchestrated by the abstract `Panel` class. `UIElement.Visibility` states significantly impact layout calculation: elements marked `Collapsed` bypass layout allocation entirely in both passes, whereas `Hidden` elements allocate space normally but skip the rendering and hit testing phases.
...
2.  **Arrange Pass:** Parents position and size their children within the computed physical bounds by invoking `Arrange(Rect finalRect)`, which applies structural offsets for `Margin`. Container controls inheriting from `Control` further apply `Padding` (`Thickness`) during their `MeasureOverride` and `ArrangeOverride` operations to reduce inner available space for their embedded `TemplateRoot`. Contrary to standard WPF, `UIElement.ArrangeOverride` returns `void` (not `Size`), and accurate control dimensions are exclusively accessed via the `UIElement.RenderSize` property, which returns a `Rect` (not `Size`) rather than `ActualWidth` or `ActualHeight`.
3.  **Render Pass:** The actual rendering to the `VirtualBuffer` is heavily optimized. Containers executing the `Render` method utilize clipping rects to skip elements that are fully clipped or lie completely outside the current clip rectangle, drastically reducing CPU cycles in complex visual trees. Templates explicitly load their visual composition by invoking `FrameworkTemplate.LoadContent(DependencyObject templatedParent)`, anchoring the visual generation to the originating component.
...
(`TuiXamlView`), Avalonia, WinUI and MAUI hosts. The `XamlLoader` utilizes a robust `ConvertValue` implementation that parses literal strings into underlying types such as `DateTime` and `TimeSpan`, enabling declarative initializations on controls like `DatePicker` and `TimePicker`. See the [XAML guide](docs/xaml.md).
